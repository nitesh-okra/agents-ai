...
function createTempConfigPath() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "Agents-allowed-hostname-"));
...
function writeBaseConfig(configPath: string) {
  const base: AgentsConfig = {
    ...
    database: {
      ...
      embeddedPostgresDataDir: "/tmp/Agents-db",
      embeddedPostgresPort: 54329,
      backup: {
        enabled: true,
        intervalMinutes: 60,
        retentionDays: 30,
        dir: "/tmp/Agents-backups",
      },
    },
    logging: {
      mode: "file",
      logDir: "/tmp/Agents-logs",
    },
    server: {
    ...
    storage: {
      provider: "local_disk",
      localDisk: { baseDir: "/tmp/Agents-storage" },
      s3: {
        bucket: "Agents",
        region: "us-east-1",
        prefix: "",
    ...
    secrets: {
      ...
      strictMode: false,
      localEncrypted: { keyFilePath: "/tmp/Agents-secrets/master.key" },
    },
  };
...
describe("allowed-hostname command", () => {
  it("adds and normalizes hostnames", async () => {
    ...
    const raw = JSON.parse(fs.readFileSync(configPath, "utf-8")) as AgentsConfig;
