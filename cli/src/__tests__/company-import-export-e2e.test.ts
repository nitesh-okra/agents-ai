...
function createBaseAgentsEnv(options: TestAgentsEnv) {
  ...
  env.Agents_CONFIG = options.configPath;
  env.Agents_HOME = options.AgentsHome;
  env.Agents_INSTANCE_ID = options.instanceId;
  env.Agents_CONTEXT = path.join(options.AgentsHome, "context.json");
  env.Agents_AUTH_STORE = path.join(options.AgentsHome, "auth.json");
...
function createServerEnv(
  ...
  options: Omit<TestAgentsEnv, "configPath">,
) {
  const env = createBaseAgentsEnv({
    configPath,
    ...options,
  });
  ...
  env.SERVE_UI = "false";
  env.Agents_DB_BACKUP_ENABLED = "false";
  env.HEARTBEAT_SCHEDULER_ENABLED = "false";
  env.Agents_MIGRATION_AUTO_APPLY = "true";
  env.Agents_UI_DEV_MIDDLEWARE = "false";

  return env;
}

function createCliEnv(options: TestAgentsEnv) {
  const env = createBaseAgentsEnv(options);
  delete env.DATABASE_URL;
  delete env.PORT;
  delete env.HOST;
  delete env.SERVE_UI;
  delete env.Agents_DB_BACKUP_ENABLED;
  delete env.HEARTBEAT_SCHEDULER_ENABLED;
  delete env.Agents_MIGRATION_AUTO_APPLY;
  delete env.Agents_UI_DEV_MIDDLEWARE;
  return env;
}
...
async function runCliJson<T>(
  args: string[],
  opts: TestAgentsEnv & { apiBase?: string; includeConfigArg?: boolean },
) {
  const repoRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../../..");
  const cliArgs = ["--silent", "Agentsai", ...args];
  if (opts.apiBase) {
...
async function waitForServer(
  ...
  while (Date.now() - startedAt < 30_000) {
    if (child.exitCode !== null) {
      throw new Error(
        `Agentsai run exited before healthcheck succeeded.\nstdout:\n${output.stdout.join("")}\nstderr:\n${output.stderr.join("")}`,
      );
    }
...
describeEmbeddedPostgres("Agentsai company import/export e2e", () => {
  let tempRoot = "";
  let configPath = "";
  let exportDir = "";
  let apiBase = "";
  let AgentsHome = "";
  let cliShellHome = "";
  let AgentsInstanceId = "";
  let serverProcess: ServerProcess | null = null;
  let tempDb: Awaited<ReturnType<typeof startEmbeddedPostgresTestDatabase>> | null = null;

  beforeAll(async () => {
    tempRoot = mkdtempSync(path.join(os.tmpdir(), "Agents-company-cli-e2e-"));
    configPath = path.join(tempRoot, "config", "config.json");
    exportDir = path.join(tempRoot, "exported-company");
    AgentsHome = path.join(tempRoot, "Agents-home");
    cliShellHome = path.join(tempRoot, "shell-home");
    AgentsInstanceId = "company-cli-e2e";
    mkdirSync(AgentsHome, { recursive: true });
    mkdirSync(cliShellHome, { recursive: true });

    tempDb = await startEmbeddedPostgresTestDatabase("Agents-company-cli-db-");
    ...
    const child = spawn(
      "pnpm",
      ["Agentsai", "run", "--config", configPath],
      {
        cwd: repoRoot,
        env: createServerEnv(configPath, port, tempDb.connectionString, {
          AgentsHome,
          instanceId: AgentsInstanceId,
  ...
  it("exports a company package and imports it into new and existing companies", async () => {
    ...
    const expectedContextPath = path.join(AgentsHome, "context.json");
    const leakedContextPath = path.join(cliShellHome, ".Agents", "context.json");
    ...
    expect(readFileSync(path.join(exportDir, ".Agents.yaml"), "utf8")).toContain('schema: "Agents/v1"');
    ...
    writeFileSync(zipPath, createStoredZipArchive(portableFiles, "Agents-demo"));
