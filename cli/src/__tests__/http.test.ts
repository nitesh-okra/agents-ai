...
import { ApiConnectionError, ApiRequestError, AgentsApiClient } from "../client/http.js";
...
describe("AgentsApiClient", () => {
  ...
  it("adds authorization and run-id headers", async () => {
    ...
    expect(headers["x-Agents-run-id"]).toBe("run-abc");
  ...
  it("returns null on ignoreNotFound", async () => {
    ...
    const client = new AgentsApiClient({ apiBase: "http://localhost:3100" });
