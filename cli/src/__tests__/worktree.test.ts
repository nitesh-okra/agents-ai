...
describe("worktree helpers", () => {
  ...
  it("copies the source local_encrypted secrets key into the seeded worktree instance", () => {
    ...
    const originalInlineMasterKey = process.env.Agents_SECRETS_MASTER_KEY;
    const originalKeyFile = process.env.Agents_SECRETS_MASTER_KEY_FILE;
    try {
      delete process.env.Agents_SECRETS_MASTER_KEY;
      delete process.env.Agents_SECRETS_MASTER_KEY_FILE;
      ...
      if (originalInlineMasterKey === undefined) {
        ...
        process.env.Agents_SECRETS_MASTER_KEY = originalInlineMasterKey;
      }
      if (originalKeyFile === undefined) {
        delete process.env.Agents_SECRETS_MASTER_KEY_FILE;
      } else {
        process.env.Agents_SECRETS_MASTER_KEY_FILE = originalKeyFile;
      }
  ...
  it("persists the current agent jwt secret into the worktree env file", async () => {
    ...
    const originalJwtSecret = process.env.Agents_AGENT_JWT_SECRET;

    try {
      fs.mkdirSync(repoRoot, { recursive: true });
      process.env.Agents_AGENT_JWT_SECRET = "worktree-shared-secret";
      ...
      if (originalJwtSecret === undefined) {
        delete process.env.Agents_AGENT_JWT_SECRET;
      } else {
        process.env.Agents_AGENT_JWT_SECRET = originalJwtSecret;
      }
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("defaults the seed source config to the current repo-local Agents config", () => {
    ...
    const originalCwd = process.cwd();
    const originalAgentsConfig = process.env.Agents_CONFIG;
    ...
    try {
      ...
      if (originalAgentsConfig === undefined) {
        ...
        process.env.Agents_CONFIG = originalAgentsConfig;
      }
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
