...
export class AgentsApiClient {
  ...
  private async request<T>(
    ...
    if (this.runId) {
      headers["x-Agents-run-id"] = this.runId;
    }
...
function buildConnectionErrorMessage(input: {
  ...
  const lines = [
    "Could not reach the Agents API.",
    "",
  ...
  lines.push(
    "",
    "This usually means the Agents server is not running, the configured URL is wrong, or the request is being blocked before it reaches Agents.",
    "",
    "Try:",
    "- Start Agents with `pnpm dev` or `pnpm Agentsai run`.",
    `- Verify the server is reachable with \`curl ${healthUrl}\`.`,
    `- If Agents is running elsewhere, pass \`--api-base ${input.apiBase.replace(/\/+$/, "")}\` or set \`Agents_API_URL\`.`,
  );
  return lines.join("\n");
}
