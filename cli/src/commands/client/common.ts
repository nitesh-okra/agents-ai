...
export function resolveCommandContext(
  ...
  const explicitApiKey =
    options.apiKey?.trim() ||
    process.env.Agents_API_KEY?.trim() ||
  ...
  const companyId =
    ...
    process.env.Agents_COMPANY_ID?.trim() ||
    profile.companyId;
...
function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.Agents_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.Agents_SERVER_PORT || "");
