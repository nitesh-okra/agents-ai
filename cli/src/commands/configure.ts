...
export async function configure(opts: {
  ...
  p.intro(pc.bgCyan(pc.black(" Agents configure ")));
  const configPath = resolveConfigPath(opts.config);

  if (!configExists(opts.config)) {
    p.log.error("No config file found. Run `Agentsai onboard` first.");
    p.outro("");
    return;
  }
      ...
      case "secrets":
        config.secrets = await promptSecrets(config.secrets);
        {
          ...
          if (keyResult.status === "created") {
            ...
            p.log.message(pc.dim("Skipping local key file management because Agents_SECRETS_MASTER_KEY is set"));
          }
        }
        break;
    }
