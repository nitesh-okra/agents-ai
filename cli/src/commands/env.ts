...
function collectDeploymentEnvRows(config: AgentsConfig | null, configPath: string): EnvVarRow[] {
  ...
  const publicUrlSource: EnvSource =
    process.env.Agents_PUBLIC_URL
      ? "env"
      : process.env.Agents_AUTH_PUBLIC_BASE_URL || process.env.BETTER_AUTH_URL || process.env.BETTER_AUTH_BASE_URL
        ? "env"
        : config?.auth?.publicBaseUrl
          ? "config"
          : "missing";
  ...
  const secretsProvider =
    process.env.Agents_SECRETS_PROVIDER ??
    config?.secrets?.provider ??
    DEFAULT_SECRETS_PROVIDER;
  const secretsStrictMode =
    process.env.Agents_SECRETS_STRICT_MODE ??
  ...
  const storageProvider =
    process.env.Agents_STORAGE_PROVIDER ??
  ...
  const storageS3Bucket =
    process.env.Agents_STORAGE_S3_BUCKET ??
    config?.storage?.s3?.bucket ??
    "Agents";
  const storageS3Region =
    process.env.Agents_STORAGE_S3_REGION ??
    config?.storage?.s3?.region ??
    "us-east-1";
  const storageS3Endpoint =
    process.env.Agents_STORAGE_S3_ENDPOINT ??
    config?.storage?.s3?.endpoint ??
    "";
  const storageS3Prefix =
    process.env.Agents_STORAGE_S3_PREFIX ??
    config?.storage?.s3?.prefix ??
    "";
  const storageS3ForcePathStyle =
    process.env.Agents_STORAGE_S3_FORCE_PATH_STYLE ??
  ...
  const rows: EnvVarRow[] = [
    ...
    },
    {
      ...
      value: process.env.Agents_AGENT_JWT_TTL_SECONDS ?? DEFAULT_AGENT_JWT_TTL_SECONDS,
      source: process.env.Agents_AGENT_JWT_TTL_SECONDS ? "env" : "default",
      required: false,
      note: "JWT lifetime in seconds",
    },
    {
      key: "Agents_AGENT_JWT_ISSUER",
      value: process.env.Agents_AGENT_JWT_ISSUER ?? DEFAULT_AGENT_JWT_ISSUER,
      source: process.env.Agents_AGENT_JWT_ISSUER ? "env" : "default",
      required: false,
      note: "JWT issuer",
    },
    {
      key: "Agents_AGENT_JWT_AUDIENCE",
      value: process.env.Agents_AGENT_JWT_AUDIENCE ?? DEFAULT_AGENT_JWT_AUDIENCE,
      source: process.env.Agents_AGENT_JWT_AUDIENCE ? "env" : "default",
      required: false,
      note: "JWT audience",
    },
    {
    ...
    },
    {
      ...
      source: process.env.Agents_SECRETS_PROVIDER
        ? "env"
        : config?.secrets?.provider
          ? "config"
          : "default",
    ...
    },
    {
      ...
      source: process.env.Agents_SECRETS_STRICT_MODE
        ? "env"
    ...
    },
    {
      ...
      source: process.env.Agents_SECRETS_MASTER_KEY_FILE
        ? "env"
        : config?.secrets?.localEncrypted?.keyFilePath
          ? "config"
          : "default",
    ...
    },
    {
      ...
      source: process.env.Agents_STORAGE_PROVIDER
        ? "env"
        : config?.storage?.provider
          ? "config"
          : "default",
    ...
    },
    {
      ...
      source: process.env.Agents_STORAGE_LOCAL_DIR
        ? "env"
        : config?.storage?.localDisk?.baseDir
          ? "config"
          : "default",
    ...
    },
    {
      ...
      source: process.env.Agents_STORAGE_S3_BUCKET
        ? "env"
        : config?.storage?.s3?.bucket
          ? "config"
          : "default",
    ...
    },
    {
      ...
      source: process.env.Agents_STORAGE_S3_REGION
        ? "env"
        : config?.storage?.s3?.region
          ? "config"
          : "default",
    ...
    },
    {
      ...
      source: process.env.Agents_STORAGE_S3_ENDPOINT
        ? "env"
        : config?.storage?.s3?.endpoint
          ? "config"
          : "default",
    ...
    },
    {
      ...
      source: process.env.Agents_STORAGE_S3_PREFIX
        ? "env"
        : config?.storage?.s3?.prefix
          ? "config"
          : "default",
    ...
    },
    {
      ...
      source: process.env.Agents_STORAGE_S3_FORCE_PATH_STYLE
        ? "env"
  ...
  if (process.env.Agents_CONFIG || configPath !== defaultConfigPath) {
    rows.push({
      key: "Agents_CONFIG",
      value: process.env.Agents_CONFIG ?? configPath,
      source: process.env.Agents_CONFIG ? "env" : "default",
      required: false,
