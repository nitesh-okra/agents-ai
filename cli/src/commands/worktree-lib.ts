...
export const DEFAULT_WORKTREE_HOME = "~/.Agents-worktrees";
...
export function resolveWorktreeLocalPaths(opts: {
  ...
  const repoConfigDir = path.resolve(cwd, ".Agents");
  return {
    cwd,
    repoConfigDir,
...
export function buildWorktreeConfig(input: {
  sourceConfig: AgentsConfig | null;
  ...
  return {
    ...
    storage: {
      ...
      s3: {
        bucket: source?.storage.s3.bucket ?? "Agents",
...
export function buildWorktreeEnvEntries(
  ...
  return {
    Agents_HOME: paths.homeDir,
    Agents_INSTANCE_ID: paths.instanceId,
    Agents_CONFIG: paths.configPath,
    Agents_CONTEXT: paths.contextPath,
    Agents_IN_WORKTREE: "true",
    ...(branding?.name ? { Agents_WORKTREE_NAME: branding.name } : {}),
    ...(branding?.color ? { Agents_WORKTREE_COLOR: branding.color } : {}),
  };
}
