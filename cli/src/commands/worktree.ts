...
function isCurrentSourceConfigPath(sourceConfigPath: string): boolean {
  const currentConfigPath = process.env.Agents_CONFIG;
...
function resolveWorktreeHome(explicit?: string): string {
  return explicit ?? process.env.Agents_WORKTREES_DIR ?? DEFAULT_WORKTREE_HOME;
}

function resolveWorktreeStartPoint(explicit?: string): string | undefined {
  return explicit ?? nonEmpty(process.env.Agents_WORKTREE_START_POINT) ?? undefined;
}
...
export function copySeededSecretsKey(input: {
  ...
  const sourceInlineMasterKey =
    ...
    (allowProcessEnvFallback ? nonEmpty(process.env.Agents_SECRETS_MASTER_KEY) : null);
  ...
  const sourceKeyFileOverride =
    ...
    (allowProcessEnvFallback ? nonEmpty(process.env.Agents_SECRETS_MASTER_KEY_FILE) : null);
...
async function runWorktreeInit(opts: WorktreeInitOptions): Promise<void> {
  ...
  const existingAgentJwtSecret =
    ...
    nonEmpty(process.env.Agents_AGENT_JWT_SECRET);
