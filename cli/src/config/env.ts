...
const JWT_SECRET_ENV_KEY = "Agents_AGENT_JWT_SECRET";
...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    ...
    "# Generated by Agents CLI commands",
...
export function resolveAgentsEnvFile(configPath?: string): string {
...
export function loadAgentsEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergeAgentsEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readAgentsEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writeAgentsEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergeAgentsEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readAgentsEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writeAgentsEnvEntries(next, filePath);
  return next;
}
