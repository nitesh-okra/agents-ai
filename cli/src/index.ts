...
const DATA_DIR_OPTION_HELP =
  "Agents data directory root (isolates state from ~/.Agents)";

program
  .name("Agentsai")
  .description("Agents CLI — setup, diagnose, and configure your instance")
  .version(cliVersion);
...
program.hook("preAction", (_thisCommand, actionCommand) => {
  ...
  loadAgentsEnvFile(options.config);
...
program
  ...
  .option("--run", "Start Agents immediately after saving config", false)
  .action(onboard);

program
  .command("doctor")
  .description("Run diagnostic checks on your Agents setup")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
program
  ...
  .option("--filename-prefix <prefix>", "Backup filename prefix", "Agents")
...
program
  .command("run")
  .description("Bootstrap local setup (onboard + doctor) and run Agents")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
heartbeat
  ...
  .option("--api-base <url>", "Base URL for the Agents server API")
