...
export function parseSandboxRemoteExecutionSpec(value: unknown): SandboxRemoteExecutionSpec | null {
  ...
  return {
    ...
    AgentsApiUrl: asString(parsed.AgentsApiUrl).trim() || null,
  };
}
...
export async function prepareSandboxManagedRuntime(input: {
  ...
  await withTempDir("Agents-sandbox-sync-", async (tempDir) => {
    ...
    const preservedNames = new Set([".Agents-runtime", ...(input.preserveAbsentOnRestore ?? [])]);
  ...
  return {
    ...
    restoreWorkspace: async () => {
      await withTempDir("Agents-sandbox-restore-", async (tempDir) => {
        ...
        await mirrorDirectory(extractedDir, input.workspaceLocalDir, {
          preserveAbsent: [".Agents-runtime", ...(input.preserveAbsentOnRestore ?? [])],
        });
      });
    },
  };
