...
Use when:
- You want Agents to run Cursor Agent CLI locally as the agent runtime
...
- Agents auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$Agents" and related skills on local runs.
- Agents auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
`;
