...
Use when:
- You want Agents to invoke OpenClaw over the Gateway WebSocket protocol.
...
- Your deployment does not permit outbound WebSocket access from the Agents server.
...
- AgentsApiUrl (string, optional): absolute Agents base URL advertised in wake text
- claimedApiKeyPath (string, optional): path to the claimed API key JSON file read by the agent at wake time (default ~/.openclaw/workspace/Agents-claimed-api-key.json)

Session routing fields:
- sessionKeyStrategy (string, optional): issue (default), fixed, or run
- sessionKey (string, optional): fixed session key when strategy=fixed (default Agents)

Standard outbound payload additions:
- Agents (object): standardized Agents context added to every gateway agent request
- Agents.workspace (object, optional): resolved execution workspace for this run
- Agents.workspaces (array, optional): additional workspace hints Agents exposed to the run
- Agents.workspaceRuntime (object, optional): reserved workspace runtime metadata when explicitly supplied outside normal heartbeat execution
