...
Use when:
- You want Agents to run Pi (the AI coding agent) locally as the agent runtime
...
- Agents requires an explicit \`model\` value for \`pi_local\` agents.
- Sessions are stored in ~/.pi/Agentss/ and resumed with --session.
