...
async function ensureEmbeddedPostgresConnection(
  ...
  const preferredAdminConnectionString = `postgres://Agents:Agents@127.0.0.1:${preferredPort}/postgres`;
  ...
  if (!runningPid && existsSync(pgVersionFile)) {
    try {
      ...
      await ensurePostgresDatabase(preferredAdminConnectionString, "Agents");
      process.emitWarning(
      ...
      return {
        connectionString: `postgres://Agents:Agents@127.0.0.1:${preferredPort}/Agents`,
  ...
  if (runningPid) {
    ...
    return {
      connectionString: `postgres://Agents:Agents@127.0.0.1:${port}/Agents`,
  ...
  const adminConnectionString = `postgres://Agents:Agents@127.0.0.1:${selectedPort}/postgres`;
  await ensurePostgresDatabase(adminConnectionString, "Agents");

  return {
    connectionString: `postgres://Agents:Agents@127.0.0.1:${selectedPort}/Agents`,
