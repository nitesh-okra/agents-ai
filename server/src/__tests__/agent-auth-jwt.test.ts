...
describe("agent local JWT", () => {
  const secretEnv = "Agents_AGENT_JWT_SECRET";
  const betterAuthSecretEnv = "BETTER_AUTH_SECRET";
  const ttlEnv = "Agents_AGENT_JWT_TTL_SECONDS";
  const issuerEnv = "Agents_AGENT_JWT_ISSUER";
  const audienceEnv = "Agents_AGENT_JWT_AUDIENCE";
  ...
  it("falls back to BETTER_AUTH_SECRET when Agents_AGENT_JWT_SECRET is absent", () => {
  ...
  it("rejects issuer/audience mismatch", () => {
    ...
    process.env[issuerEnv] = "Agents";
    process.env[audienceEnv] = "Agents-api";
    expect(verifyLocalAgentJwt(token!)).toBeNull();
  });
});
