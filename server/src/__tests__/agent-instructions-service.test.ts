...
describe("agent instructions service", () => {
  ...
  const originalAgentsInstanceId = process.env.Agents_INSTANCE_ID;
  const cleanupDirs = new Set<string>();

  afterEach(async () => {
    if (originalAgentsHome === undefined) delete process.env.Agents_HOME;
    else process.env.Agents_HOME = originalAgentsHome;
    if (originalAgentsInstanceId === undefined) delete process.env.Agents_INSTANCE_ID;
    else process.env.Agents_INSTANCE_ID = originalAgentsInstanceId;
  ...
  it("copies the existing bundle into the managed root when switching to managed mode", async () => {
    ...
    process.env.Agents_INSTANCE_ID = "test-instance";
