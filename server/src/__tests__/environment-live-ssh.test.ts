...
function resolveEnvLabStatePath(): string {
  const instanceRoot =
    process.env.Agents_INSTANCE_ROOT?.trim() ||
...
function tryExplicitConfig(): {
  ...
  const host = process.env.Agents_ENV_LIVE_SSH_HOST?.trim() ?? "";
  const username = process.env.Agents_ENV_LIVE_SSH_USERNAME?.trim() ?? "";
  const remoteWorkspacePath =
    process.env.Agents_ENV_LIVE_SSH_REMOTE_WORKSPACE_PATH?.trim() ?? "";
  const port = Number.parseInt(process.env.Agents_ENV_LIVE_SSH_PORT ?? "22", 10);
...
async function resolveSshConfig(): Promise<SshConnectionConfig | null> {
  ...
  if (explicit) {
    return {
      ...
      privateKey: await readOptionalSecret(
        process.env.Agents_ENV_LIVE_SSH_PRIVATE_KEY,
        process.env.Agents_ENV_LIVE_SSH_PRIVATE_KEY_PATH,
      ),
      knownHosts: await readOptionalSecret(
        process.env.Agents_ENV_LIVE_SSH_KNOWN_HOSTS,
        process.env.Agents_ENV_LIVE_SSH_KNOWN_HOSTS_PATH,
      ),
      strictHostKeyChecking:
        (process.env.Agents_ENV_LIVE_SSH_STRICT_HOST_KEY_CHECKING ?? "true").toLowerCase() !== "false",
    };
  }
  ...
  if (process.env.Agents_ENV_LIVE_SSH_NO_AUTO_FIXTURE !== "true") {
