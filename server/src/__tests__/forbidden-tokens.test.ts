...
describe("forbidden token check", () => {
  it("derives username tokens without relying on whoami", () => {
    const tokens = resolveDynamicForbiddenTokens(
      { USER: "Agents", LOGNAME: "Agents", USERNAME: "pc" },
      {
        userInfo: () => ({ username: "Agents" }),
      },
    );

    expect(tokens).toEqual(["Agents", "pc"]);
  });
  ...
  it("merges dynamic and file-based forbidden tokens", async () => {
    ...
    try {
      const tokens = resolveForbiddenTokens(tokensFile, { USER: "Agents" }, {
        userInfo: () => ({ username: "Agents" }),
      });

      expect(tokens).toEqual(["Agents", "custom-token"]);
    } finally {
