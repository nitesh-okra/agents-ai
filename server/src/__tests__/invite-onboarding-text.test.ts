...
describe("buildInviteOnboardingTextDocument", () => {
  it("renders a plain-text onboarding doc with expected endpoint references", () => {
    ...
    expect(text).toContain("Agents OpenClaw Gateway Onboarding");
    ...
    expect(text).toContain("/api/invites/token-123/skills/Agents");
    expect(text).toContain("Suggested Agents base URLs to try");
    expect(text).toContain("http://localhost:3100");
    expect(text).toContain("host.docker.internal");
    expect(text).toContain("AgentsApiUrl");
    ...
    expect(text).toContain("set the first reachable candidate as agentDefaultsPayload.AgentsApiUrl");
    expect(text).toContain("~/.openclaw/workspace/Agents-claimed-api-key.json");
    expect(text).toContain("Agents_API_KEY");
