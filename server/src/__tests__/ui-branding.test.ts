...
<head>
    <!-- Agents_RUNTIME_BRANDING_START -->
    <!-- Agents_RUNTIME_BRANDING_END -->
    <!-- Agents_FAVICON_START -->
...
describe("ui branding", () => {
  it("detects worktree mode from Agents_IN_WORKTREE", () => {
    expect(isWorktreeUiBrandingEnabled({ Agents_IN_WORKTREE: "true" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ Agents_IN_WORKTREE: "1" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ Agents_IN_WORKTREE: "false" })).toBe(false);
  });

  it("resolves name, color, and text color for worktree branding", () => {
    const branding = getWorktreeUiBranding({
      Agents_IN_WORKTREE: "true",
      Agents_WORKTREE_NAME: "Agents-pr-432",
      Agents_WORKTREE_COLOR: "#4f86f7",
    });

    expect(branding.enabled).toBe(true);
    expect(branding.name).toBe("Agents-pr-432");
  ...
  it("renders runtime branding metadata for the ui", () => {
    ...
    expect(meta).toContain('name="Agents-worktree-name"');
    expect(meta).toContain('content="Agents-pr-432"');
    expect(meta).toContain('name="Agents-worktree-color"');
  });
  ...
  it("rewrites the favicon and runtime branding blocks for worktree instances only", () => {
    ...
    expect(branded).toContain('name="Agents-worktree-name"');
    ...
    expect(defaultHtml).not.toContain('name="Agents-worktree-name"');
  });
});
