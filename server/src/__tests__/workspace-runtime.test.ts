...
afterEach(async () => {
  ...
  delete process.env.Agents_WORKTREES_DIR;
...
describe("realizeExecutionWorkspace", () => {
  ...
  it("writes an isolated repo-local Agents config and worktree branding when provisioning", async () => {
    ...
    process.env.Agents_WORKTREES_DIR = isolatedWorktreeHome;
...
describe("ensureRuntimeServicesForRun", () => {
  ...
  it("does not reuse project-scoped shared services across different workspace launch contexts", async () => {
    ...
    const serviceCommand =
      "node -e \"require('node:http').createServer((req,res)=>res.end(process.env.Agents_HOME)).listen(Number(process.env.PORT), '127.0.0.1')\"";
    const config = {
      workspaceRuntime: {
        services: [
          {
  ...
  it("does not leak parent Agents instance env into runtime service commands", async () => {
    ...
    const serviceCommand = [
      ...
      JSON.stringify(
        [
          ...
          "AgentsConfig: process.env.Agents_CONFIG ?? null,",
          "AgentsHome: process.env.Agents_HOME ?? null,",
          "AgentsInstanceId: process.env.Agents_INSTANCE_ID ?? null,",
    ...
    process.env.Agents_CONFIG = "/tmp/base-Agents-config.json";
    process.env.Agents_HOME = "/tmp/base-Agents-home";
    process.env.Agents_INSTANCE_ID = "base-instance";
...
describeEmbeddedPostgres("workspace runtime startup reconciliation", () => {
  ...
  it("adopts a live auto-port shared service after runtime state is reset", async () => {
    ...
    process.env.Agents_HOME = AgentsHome;
    process.env.Agents_INSTANCE_ID = `runtime-reconcile-${randomUUID()}`;
