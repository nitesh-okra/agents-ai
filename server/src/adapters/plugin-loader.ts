...
function extractUiParserSource(
  ...
  const contractVersion = pkg.Agents?.adapterUiParser;
  if (contractVersion) {
    ...
    logger.info(
      { packageName },
      "Adapter has ./ui-parser export but no Agents.adapterUiParser version — loading anyway (future versions may require it)",
    );
  }
