...
function jwtConfig() {
  const secret = process.env.Agents_AGENT_JWT_SECRET?.trim() || process.env.BETTER_AUTH_SECRET?.trim();
  if (!secret) return null;

  return {
    secret,
    ttlSeconds: parseNumber(process.env.Agents_AGENT_JWT_TTL_SECONDS, 60 * 60 * 48),
    issuer: process.env.Agents_AGENT_JWT_ISSUER ?? "Agents",
    audience: process.env.Agents_AGENT_JWT_AUDIENCE ?? "Agents-api",
  };
}
