...
export async function startServer(): Promise<StartedServer> {
  ...
  if (process.env.Agents_SECRETS_PROVIDER === undefined) {
    process.env.Agents_SECRETS_PROVIDER = config.secretsProvider;
  }
  if (process.env.Agents_SECRETS_STRICT_MODE === undefined) {
    process.env.Agents_SECRETS_STRICT_MODE = config.secretsStrictMode ? "true" : "false";
  }
  if (process.env.Agents_SECRETS_MASTER_KEY_FILE === undefined) {
    process.env.Agents_SECRETS_MASTER_KEY_FILE = config.secretsMasterKeyFilePath;
  }
  ...
  async function promptApplyMigrations(migrations: string[]): Promise<boolean> {
    if (process.env.Agents_MIGRATION_AUTO_APPLY === "true") return true;
    if (process.env.Agents_MIGRATION_PROMPT === "never") return false;
  ...
  if (config.databaseUrl) {
    ...
    const verboseEmbeddedPostgresLogs = process.env.Agents_EMBEDDED_POSTGRES_VERBOSE === "true";
  ...
  const configuredApiUrl = process.env.Agents_API_URL?.trim() || runtimeApiUrl;
  process.env.Agents_LISTEN_HOST = runtimeListenHost;
  process.env.Agents_LISTEN_PORT = String(listenPort);
  process.env.Agents_RUNTIME_API_URL = runtimeApiUrl;
  process.env.Agents_RUNTIME_API_CANDIDATES_JSON = JSON.stringify(runtimeApiCandidates);
  process.env.Agents_API_URL = configuredApiUrl;
  ...
  await new Promise<void>((resolveListen, rejectListen) => {
    ...
    server.listen(listenPort, config.host, () => {
      ...
      if (process.env.Agents_OPEN_ON_LISTEN === "true") {
