 ...
 * ~/.Agents/adapter-plugins.json. This is the source of truth for which
...
import { resolveAgentsHomeDir } from "../home-paths.js";
...
export interface AdapterPluginRecord {
  /** npm package name (e.g., "droid-Agents-adapter") */
  packageName: string;
...
function adapterPluginPaths() {
  const AgentsDir = resolveAgentsHomeDir();
  return {
    adapterPluginsDir: path.join(AgentsDir, "adapter-plugins"),
    adapterPluginsStorePath: path.join(AgentsDir, "adapter-plugins.json"),
    adapterSettingsPath: path.join(AgentsDir, "adapter-settings.json"),
  };
}
...
function ensureDirs(): string {
  ...
  if (!fs.existsSync(pkgJsonPath)) {
    fs.writeFileSync(pkgJsonPath, JSON.stringify({
      name: "Agents-adapter-plugins",
      version: "0.0.0",
      private: true,
      description: "Managed directory for Agents external adapter plugins. Do not edit manually.",
    }, null, 2) + "\n");
  }
