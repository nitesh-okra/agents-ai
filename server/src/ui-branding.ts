const FAVICON_BLOCK_START = "<!-- Agents_FAVICON_START -->";
const FAVICON_BLOCK_END = "<!-- Agents_FAVICON_END -->";
const RUNTIME_BRANDING_BLOCK_START = "<!-- Agents_RUNTIME_BRANDING_START -->";
const RUNTIME_BRANDING_BLOCK_END = "<!-- Agents_RUNTIME_BRANDING_END -->";
...
export function isWorktreeUiBrandingEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return isTruthyEnvValue(env.Agents_IN_WORKTREE);
}
...
export function getWorktreeUiBranding(env: NodeJS.ProcessEnv = process.env): WorktreeUiBranding {
  ...
  const name = nonEmpty(env.Agents_WORKTREE_NAME) ?? nonEmpty(env.Agents_INSTANCE_ID) ?? "worktree";
  const color = normalizeHexColor(env.Agents_WORKTREE_COLOR) ?? deriveColorFromSeed(name);
...
export function renderRuntimeBrandingMeta(branding: WorktreeUiBranding): string {
  ...
  return [
    '<meta name="Agents-worktree-enabled" content="true" />',
    `<meta name="Agents-worktree-name" content="${escapeHtmlAttribute(branding.name)}" />`,
    `<meta name="Agents-worktree-color" content="${escapeHtmlAttribute(branding.color)}" />`,
    `<meta name="Agents-worktree-text-color" content="${escapeHtmlAttribute(branding.textColor)}" />`,
  ].join("\n");
}
