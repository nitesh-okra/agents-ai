...
export function OpenClawGatewayConfigFields({
  ...
  return (
    <>
      ...
      {!isCreate && (
        <>
          <Field label="Agents API URL override">
            <DraftInput
              value={
                eff(
                  "adapterConfig",
                  "AgentsApiUrl",
                  String(config.AgentsApiUrl ?? ""),
                )
              }
              onCommit={(v) => mark("adapterConfig", "AgentsApiUrl", v || undefined)}
              immediate
              className={inputClass}
              placeholder="https://Agents.example"
            />
          </Field>
          ...
          <Field label="Claimed API key path">
            <DraftInput
              ...
              immediate
              className={inputClass}
              placeholder="~/.openclaw/workspace/Agents-claimed-api-key.json"
            />
          </Field>
          ...
          {sessionStrategy === "fixed" && (
            <Field label="Session key">
              <DraftInput
                value={eff("adapterConfig", "sessionKey", String(config.sessionKey ?? "Agents"))}
          ...
          <Field label="Device auth">
            <div className="text-xs text-muted-foreground leading-relaxed">
              Always enabled for gateway agents. Agents persists a device key during onboarding so pairing approvals
