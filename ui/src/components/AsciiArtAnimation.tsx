...
type AgentsSprite = (typeof Agents_SPRITES)[number];
...
function spriteSize(sprite: AgentsSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = Agents_SPRITES[(baseCol + baseRow) % Agents_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = Agents_SPRITES[Math.floor(Math.random() * Agents_SPRITES.length)]!;
