...
function MarkdownIssueLink({
  ...
  return (
    <Link
      ...
      className="Agents-markdown-issue-ref"
      title={title}
...
function MermaidDiagramBlock({ source, darkMode }: { source: string; darkMode: boolean }) {
  ...
  useEffect(() => {
    ...
    loadMermaid()
      .then(async (mermaid) => {
        ...
        const rendered = await mermaid.render(`Agents-mermaid-${renderId}`, source);
        if (!active) return;
        setSvg(rendered.svg);
      })
      .catch((err) => {
  ...
  return (
    <div className="Agents-mermaid">
      {svg ? (
        <div dangerouslySetInnerHTML={{ __html: svg }} />
      ) : (
        <>
          <p className={cn("Agents-mermaid-status", error && "Agents-mermaid-status-error")}>
            {error ? `Unable to render Mermaid diagram: ${error}` : "Rendering Mermaid diagram..."}
          </p>
          <pre className="Agents-mermaid-source">
...
export function MarkdownBody({
      ...
      if (parsed) {
        ...
        return (
          <a
            ...
            className={cn(
              "Agents-mention-chip",
              `Agents-mention-chip--${parsed.kind}`,
              parsed.kind === "project" && "Agents-project-mention-chip",
            )}
  ...
  return (
    <div
      className={cn(
        "Agents-markdown prose prose-sm min-w-0 max-w-full break-words overflow-hidden",
