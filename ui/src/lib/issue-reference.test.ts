...
describe("issue-reference", () => {
  ...
  it("does not treat GitHub issue URLs as internal Agents issue links", () => {
    expect(parseIssuePathIdFromPath("https://github.com/Agentsai/Agents/issues/1778")).toBeNull();
    expect(parseIssueReferenceFromHref("https://github.com/Agentsai/Agents/issues/1778")).toBeNull();
  });
  ...
  it("preserves absolute Agents issue URLs so origin, port, and hash are not lost", () => {
