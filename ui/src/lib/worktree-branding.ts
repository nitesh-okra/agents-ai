...
export function getWorktreeUiBranding(): WorktreeUiBranding | null {
  if (readMetaContent("Agents-worktree-enabled") !== "true") return null;

  const name = readMetaContent("Agents-worktree-name");
  const color = normalizeHexColor(readMetaContent("Agents-worktree-color"));
  ...
  return {
    ...
    name,
    color,
    textColor: normalizeHexColor(readMetaContent("Agents-worktree-text-color")) ?? pickReadableTextColor(color),
  };
}
